/**
 * Tipos TypeScript desta feature — extensão dos contratos já definidos em
 * specs/001-vitrine-catalogo/contracts/types.ts. Gerado durante o
 * planejamento — NÃO é código de produção ainda.
 *
 * Mapeamento para a árvore real:
 *   PurchaseMode (novo)                          → features/products/types.ts (extensão de Product)
 *   PartnerApplication, PartnerApplicationSummary → features/partner-applications/types.ts
 *   SubmitPartnerApplicationInput                 → features/partner-applications/types.ts
 *   ActionResult                                   → shared/types.ts (já existente, reaproveitado)
 */

// ---------------------------------------------------------------------------
// Extensão de Product (dono: features/products/types.ts)
// ---------------------------------------------------------------------------

export type PurchaseMode = "mercado_livre" | "formulario_parceria";

// `Product` (specs/001) e `CreateProductInput`/`UpdateProductInput` (specs/001)
// ganham o campo:
//   purchase_mode: PurchaseMode;

// ---------------------------------------------------------------------------
// Domínio público — dono: features/partner-applications/types.ts
// ---------------------------------------------------------------------------
