  ...
  return (
    <label style={{ display: 'inline-flex', alignItems: 'center', gap: 10, cursor: disabled ? 'not-allowed' : 'pointer', opacity: disabled ? 0.5 : 1, userSelect: 'none', ...style }}>
      <span role="switch" aria-checked={checked} onClick={() => !disabled && onChange && onChange(!checked)}
        style={{ position: 'relative', width: w, height: h, flex: 'none', borderRadius: 999, background: checked ? 'var(--nerta-blue)' : 'var(--navy-light)', border: '1px solid ' + (checked ? 'transparent' : 'var(--border-subtle)'), transition: 'background var(--duration-fast)' }}>
...
function UIModal({ open, title, children, onClose, onConfirm, confirmLabel = 'Confirmar', cancelLabel = 'Cancelar', destructive = false, hideFooter = false, width = 440 }) {
  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 200, background: 'var(--scrim)', backdropFilter: 'blur(3px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div onClick={(e) => e.stopPropagation()} style={{ width: '100%', maxWidth: width, background: 'var(--surface-card)', border: '1px solid var(--border-subtle)', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow-lg)', padding: 26 }}>
        ...
        {!hideFooter && (
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, marginTop: 26 }}>
            ...
            {destructive ? (
              <button onClick={onConfirm} onMouseEnter={(e) => { e.currentTarget.style.background = '#E0573F'; }} onMouseLeave={(e) => { e.currentTarget.style.background = '#D64C39'; }}
    ...
    <div style={{
      ...
      {tags.map((t) => (
        <span key={t} style={{
          ...
          {t}
          <button onClick={() => onChange(tags.filter((x) => x !== t))} style={{ display: 'inline-flex', background: 'none', border: 'none', color: 'var(--teal)', cursor: 'pointer', padding: 2 }}>
  ...
  return (
    <div style={{ position: 'relative', display: 'inline-flex' }}>
      <select value={value} onChange={(e) => onChange(e.target.value)} style={{
        ...
        {options.map((o) => <option key={o} value={o} style={{ background: '#112644' }}>{o}</option>)}
      </select>
